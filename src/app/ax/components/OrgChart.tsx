                ...
                <div className="lg:col-span-7 bg-white/60 border border-slate-200 rounded-2xl p-6 flex flex-col justify-between space-y-8 min-h-[500px]">
                    ...
                    <div className="flex justify-center">
                        <button 
                            ...
                            className={`px-8 py-3 rounded-xl font-extrabold shadow-md transition-all ${selectedNode === 'ciso' ? 'bg-indigo-600 border-2 border-indigo-400 text-slate-900 scale-105' : 'bg-slate-100 border-2 border-indigo-500 text-slate-900 hover:bg-slate-700 hover:scale-105'}`}
                        >
                            C I S O
                        </button>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-8">
                        ...
                        {/* Legacy Track Group */}
                        <div className="space-y-4 border border-rose-950/40 p-4 rounded-xl bg-slate-50/20">
                            ...
                            <button 
                                ...
                                className={`w-full py-3 rounded-lg text-sm transition-all ${selectedNode === 'legacy' ? 'bg-rose-950/40 border-2 border-rose-400 text-rose-100 scale-105' : 'bg-slate-50 border border-rose-900 hover:border-rose-700 text-rose-200 hover:bg-rose-950/10 hover:scale-[1.02]'}`}
                            >
                                Legacy 보안팀
                            </button>
                            <div className="space-y-2 pl-2">
                                <div className="text-[11px] text-slate-500 border-l-2 border-slate-300 pl-2">인프라/실물 보안</div>
                                <div className="text-[11px] text-slate-500 border-l-2 border-slate-300 pl-2">기존 법적 규제 준수</div>
                                <div className="text-[11px] text-slate-500 border-l-2 border-slate-300 pl-2">전통 SOC 관제</div>
                            </div>
                        </div>

                        {/* AX Track Group */}
                        <div className="space-y-4 border border-indigo-950/40 p-4 rounded-xl bg-slate-50/30">
                            ...
                            <button 
                                ...
                                className={`w-full py-3 rounded-lg text-sm font-semibold shadow-lg shadow-indigo-950/50 transition-all ${selectedNode === 'ax_main' ? 'bg-indigo-600 border-2 border-indigo-400 text-slate-900 scale-105' : 'bg-indigo-950/50 border-2 border-indigo-600 text-indigo-100 hover:bg-indigo-900/50 hover:scale-[1.02]'}`}
                            >
                                AX 보안팀 (신설 핵심)
                            </button>
                            <div className="grid grid-cols-2 gap-2">
                                {['ax_policy', 'ax_privacy', 'ax_eng', 'ax_cloud'].map((id) => (
                                    <button 
                                        ...
                                        className={`py-2 rounded text-[11px] transition-all ${selectedNode === id ? 'bg-indigo-600 border border-indigo-400 text-slate-900 scale-105' : 'bg-slate-50 border border-indigo-900 hover:border-indigo-700 text-indigo-300 hover:scale-105'}`}
                                    >
                    ...
                    <div className="flex justify-center">
                        <button 
                            ...
                            className={`w-full max-w-[80%] py-3.5 rounded-xl text-sm font-extrabold shadow-lg transition-all flex items-center justify-center space-x-2 ${selectedNode === 'champions' ? 'bg-emerald-600 border-2 border-emerald-400 text-slate-900 scale-105 shadow-emerald-950/50' : 'bg-gradient-to-r from-emerald-600 to-indigo-600 text-slate-900 hover:scale-105 shadow-emerald-950/50'}`}
                        >
